                ...
                <tbody>
                    {items.map(it => (
                        <tr key={it.id} style={{ borderBottom: '1px solid var(--csp-border)' }}>
                            ...
                            <td style={styles.td}>
                                <button type="button" onClick={() => setForm({ lang: it.lang, namespace: it.namespace, key: it.key, value: it.value })} style={styles.linkBtn}>Edit</button>
                                <span style={{ margin: '0 5px', color: '#ccc' }}>|</span>
                                <button type="button" onClick={() => remove(it.id)} style={{ ...styles.linkBtn, color: 'red' }}>Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            </table>
            {items.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: '#888' }}>No translations found.</div>}
        </div>
      </div>
    </div>
  )
